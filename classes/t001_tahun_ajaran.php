...
/**
 * Table class for t001_tahun_ajaran
 */
class t001_tahun_ajaran extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't001_tahun_ajaran';
		$this->TableName = 't001_tahun_ajaran';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t001_tahun_ajaran`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t001_tahun_ajaran', 't001_tahun_ajaran', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Mulai
		$this->Mulai = new DbField('t001_tahun_ajaran', 't001_tahun_ajaran', 'x_Mulai', 'Mulai', '`Mulai`', '`Mulai`', 200, -1, FALSE, '`Mulai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Selesai
		$this->Selesai = new DbField('t001_tahun_ajaran', 't001_tahun_ajaran', 'x_Selesai', 'Selesai', '`Selesai`', '`Selesai`', 200, -1, FALSE, '`Selesai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Aktif
		$this->Aktif = new DbField('t001_tahun_ajaran', 't001_tahun_ajaran', 'x_Aktif', 'Aktif', '`Aktif`', '`Aktif`', 202, -1, FALSE, '`Aktif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->Aktif->Nullable = FALSE; // NOT NULL field
		$this->Aktif->Sortable = TRUE; // Allow sort
		$this->Aktif->DataType = DATATYPE_BOOLEAN;
		$this->Aktif->Lookup = new Lookup('Aktif', 't001_tahun_ajaran', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t001_tahun_ajaran`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t001_tahun_ajaranlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "t001_tahun_ajaranview.php")
			return $Language->phrase("View");
		elseif ($pageName == "t001_tahun_ajaranedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t001_tahun_ajaranadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t001_tahun_ajaranview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t001_tahun_ajaranview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t001_tahun_ajaranadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t001_tahun_ajaranadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t001_tahun_ajaranedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t001_tahun_ajaranadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t001_tahun_ajarandelete.php", $this->getUrlParm());
	}
	...
	public function renderListRow()
	{
		...
		if (ConvertToBool($this->Aktif->CurrentValue)) {
			$this->Aktif->ViewValue = $this->Aktif->tagCaption(2) <> "" ? $this->Aktif->tagCaption(2) : "1";
		} else {
			$this->Aktif->ViewValue = $this->Aktif->tagCaption(1) <> "" ? $this->Aktif->tagCaption(1) : "0";
		}
