...
/**
 * Table class for t102_daf_kelas_siswa
 */
class t102_daf_kelas_siswa extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't102_daf_kelas_siswa';
		$this->TableName = 't102_daf_kelas_siswa';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t102_daf_kelas_siswa`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t102_daf_kelas_siswa', 't102_daf_kelas_siswa', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// daf_kelas_id
		$this->daf_kelas_id = new DbField('t102_daf_kelas_siswa', 't102_daf_kelas_siswa', 'x_daf_kelas_id', 'daf_kelas_id', '`daf_kelas_id`', '`daf_kelas_id`', 3, -1, FALSE, '`daf_kelas_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->daf_kelas_id->IsForeignKey = TRUE; // Foreign key field
		...
		// siswa_id
		$this->siswa_id = new DbField('t102_daf_kelas_siswa', 't102_daf_kelas_siswa', 'x_siswa_id', 'siswa_id', '`siswa_id`', '`siswa_id`', 3, -1, FALSE, '`siswa_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t101_daf_kelas") {
			if ($this->daf_kelas_id->getSessionValue() <> "")
				$masterFilter .= "`id`=" . QuotedValue($this->daf_kelas_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t101_daf_kelas") {
			if ($this->daf_kelas_id->getSessionValue() <> "")
				$detailFilter .= "`daf_kelas_id`=" . QuotedValue($this->daf_kelas_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_t101_daf_kelas()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_t101_daf_kelas()
	{
		return "`daf_kelas_id`=@daf_kelas_id@";
	}

	// Table level SQL
	public function getSqlFrom() // From
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t102_daf_kelas_siswa`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t102_daf_kelas_siswalist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t102_daf_kelas_siswaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t102_daf_kelas_siswaadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t102_daf_kelas_siswaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t102_daf_kelas_siswaview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t102_daf_kelas_siswaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t102_daf_kelas_siswaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t102_daf_kelas_siswaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t102_daf_kelas_siswaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t102_daf_kelas_siswadelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "t101_daf_kelas" && !ContainsString($url, TABLE_SHOW_MASTER . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->daf_kelas_id->CurrentValue);
		}
		return $url;
	}
