...
/**
 * Table class for v103_daf_kelas_siswa_iuran
 */
class v103_daf_kelas_siswa_iuran extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v103_daf_kelas_siswa_iuran';
		$this->TableName = 'v103_daf_kelas_siswa_iuran';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v103_daf_kelas_siswa_iuran`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// NomorInduk
		$this->NomorInduk = new DbField('v103_daf_kelas_siswa_iuran', 'v103_daf_kelas_siswa_iuran', 'x_NomorInduk', 'NomorInduk', '`NomorInduk`', '`NomorInduk`', 200, -1, FALSE, '`NomorInduk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NomorInduk->Sortable = TRUE; // Allow sort
		$this->fields['NomorInduk'] = &$this->NomorInduk;

		// Nama
		$this->Nama = new DbField('v103_daf_kelas_siswa_iuran', 'v103_daf_kelas_siswa_iuran', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`v103_daf_kelas_siswa_iuran`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "v103_daf_kelas_siswa_iuranlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "v103_daf_kelas_siswa_iuranview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v103_daf_kelas_siswa_iuranedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v103_daf_kelas_siswa_iuranadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("v103_daf_kelas_siswa_iuranview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v103_daf_kelas_siswa_iuranview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "v103_daf_kelas_siswa_iuranadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v103_daf_kelas_siswa_iuranadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v103_daf_kelas_siswa_iuranedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v103_daf_kelas_siswa_iuranadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v103_daf_kelas_siswa_iurandelete.php", $this->getUrlParm());
	}
	...
	{
		$table = 'v103_daf_kelas_siswa_iuran';
		$usr = CurrentUserID();
