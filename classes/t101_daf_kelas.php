...
/**
 * Table class for t101_daf_kelas
 */
class t101_daf_kelas extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't101_daf_kelas';
		$this->TableName = 't101_daf_kelas';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t101_daf_kelas`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t101_daf_kelas', 't101_daf_kelas', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// tahun_ajaran_id
		$this->tahun_ajaran_id = new DbField('t101_daf_kelas', 't101_daf_kelas', 'x_tahun_ajaran_id', 'tahun_ajaran_id', '`tahun_ajaran_id`', '`tahun_ajaran_id`', 3, -1, FALSE, '`tahun_ajaran_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->tahun_ajaran_id->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->tahun_ajaran_id->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		$this->tahun_ajaran_id->Lookup = new Lookup('tahun_ajaran_id', 't001_tahun_ajaran', FALSE, 'id', ["Mulai","Selesai","",""], [], [], [], [], [], [], '', '');
		...
		// sekolah_id
		$this->sekolah_id = new DbField('t101_daf_kelas', 't101_daf_kelas', 'x_sekolah_id', 'sekolah_id', '`sekolah_id`', '`sekolah_id`', 3, -1, FALSE, '`sekolah_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->sekolah_id->Lookup = new Lookup('sekolah_id', 't002_sekolah', FALSE, 'id', ["Nama","","",""], [], [], [], [], [], [], '', '');
		$this->sekolah_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['sekolah_id'] = &$this->sekolah_id;

		// kelas_id
		$this->kelas_id = new DbField('t101_daf_kelas', 't101_daf_kelas', 'x_kelas_id', 'kelas_id', '`kelas_id`', '`kelas_id`', 3, -1, FALSE, '`kelas_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->kelas_id->Lookup = new Lookup('kelas_id', 't003_kelas', FALSE, 'id', ["Nama","","",""], [], [], [], [], [], [], '', '');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "t102_daf_kelas_siswa") {
			$detailUrl = $GLOBALS["t102_daf_kelas_siswa"]->getListUrl() . "?" . TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "t101_daf_kelaslist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t101_daf_kelas`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 't102_daf_kelas_siswa'
		$cascadeUpdate = FALSE;
		$rscascade = array();
		if ($rsold && (isset($rs['id']) && $rsold['id'] <> $rs['id'])) { // Update detail field 'daf_kelas_id'
			$cascadeUpdate = TRUE;
			$rscascade['daf_kelas_id'] = $rs['id']; 
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["t102_daf_kelas_siswa"]))
				$GLOBALS["t102_daf_kelas_siswa"] = new t102_daf_kelas_siswa();
			$rswrk = $GLOBALS["t102_daf_kelas_siswa"]->loadRs("`daf_kelas_id` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["t102_daf_kelas_siswa"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["t102_daf_kelas_siswa"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["t102_daf_kelas_siswa"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 't102_daf_kelas_siswa'
		if (!isset($GLOBALS["t102_daf_kelas_siswa"]))
			$GLOBALS["t102_daf_kelas_siswa"] = new t102_daf_kelas_siswa();
		$rscascade = $GLOBALS["t102_daf_kelas_siswa"]->loadRs("`daf_kelas_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB")); 
		$dtlrows = ($rscascade) ? $rscascade->getRows() : array();

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["t102_daf_kelas_siswa"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["t102_daf_kelas_siswa"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["t102_daf_kelas_siswa"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t101_daf_kelasedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t101_daf_kelasadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t101_daf_kelasview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_daf_kelasview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t101_daf_kelasadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t101_daf_kelasadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t101_daf_kelasedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_daf_kelasedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t101_daf_kelasadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t101_daf_kelasadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t101_daf_kelasdelete.php", $this->getUrlParm());
	}
