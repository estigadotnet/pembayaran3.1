...
/**
 * Table class for t302_userlevels
 */
class t302_userlevels extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't302_userlevels';
		$this->TableName = 't302_userlevels';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t302_userlevels`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// userlevelid
		$this->userlevelid = new DbField('t302_userlevels', 't302_userlevels', 'x_userlevelid', 'userlevelid', '`userlevelid`', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// userlevelname
		$this->userlevelname = new DbField('t302_userlevels', 't302_userlevels', 'x_userlevelname', 'userlevelname', '`userlevelname`', '`userlevelname`', 200, -1, FALSE, '`userlevelname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t302_userlevels`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t302_userlevelslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "t302_userlevelsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "t302_userlevelsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t302_userlevelsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t302_userlevelsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t302_userlevelsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t302_userlevelsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t302_userlevelsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t302_userlevelsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t302_userlevelsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t302_userlevelsdelete.php", $this->getUrlParm());
	}
